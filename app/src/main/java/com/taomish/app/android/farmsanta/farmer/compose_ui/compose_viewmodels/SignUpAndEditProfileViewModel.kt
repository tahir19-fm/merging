...
class SignUpAndEditProfileViewModel : ViewModel() {
    ...
    fun loadFarmerDetails(context: Context) {
        ...
        mobileNumber.postValue(selectedFarmer.value?.mobile?.removePrefix(cCode) ?: "")
    ...
    fun getFarmer(context: Context): Farmer {
        ...
        val mobileNo = (AppPrefs(context).countryCode + mobileNumber.value)
            ...
            override fun onTaskSuccess(data: Any?) {
                if (data is Array<*>) {
                    ...
                    if (!list.isNullOrEmpty()) {
                        ...
                        list.find { it.name?.lowercase() == selectedFarmer.value?.gender?.lowercase() }?.let {
